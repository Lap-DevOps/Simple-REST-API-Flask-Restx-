...
class LikeAnalytic(Resource):
    ...
    @analytics_namespace.doc(responses={200: "Success", 500: "Internal Server Error"}, security="jsonWebToken")
    ...
    def get(self):
        ...
        try:
            ...
            result = db.session.query(
                ...
                Like.created_at.between(date_from, date_to)
            ).group_by(
                func.date(Like.created_at)
            ).all()

            # Convert the result to a dictionary for the response
            analytics_data = [{'date': row.date.strftime('%Y-%m-%d'), 'like_count': row.like_count} for row in result]
...
class UserAnalytic(Resource):
    ...
    def get(self, user_id):
        ...
        response = {
            ...
            'last_api_request': user.last_api_request.strftime('%Y-%m-%d %H:%M:%S') if user.last_api_request else None
        }
